...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	err_msg := ""
	v := m.Command
	logger.Warn(fmt.Sprintf("check server %d log, value is %v", i, v))
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			//log.Printf("%v: log %v \n; server %v\n", i, cfg.logs[i], cfg.logs[j])
			logger.Warn(fmt.Sprintf("%v: log %v \n; server %v\n", i, cfg.logs[i], cfg.logs[j]))
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	// create the client name, which is used to communicate with server
	for j := 0; j < cfg.n; j++ {
	...
	for j := 0; j < cfg.n; j++ {
		// create and Client by client name, connect it to corresponding server
		// The server is denoted by a number
	...
	// create raft server of index i
	// ends is the Client set already exists
	...
	// bind the handler of raft server to service
