...
func TestReElection2A(t *testing.T) {
	...
	logger.Info(fmt.Sprintf("checkOneleader %d", leader1))
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	logger.Info(fmt.Sprintf("disconnect leader %d", leader1))
	cfg.checkOneLeader()
	...
	cfg.connect(leader1)
	logger.Info(fmt.Sprintf("connect leader %d", leader1))
	leader2 := cfg.checkOneLeader()
	logger.Info(fmt.Sprintf("checkOneleader %d", leader2))
	// if there's no quorum, no new leader should
	// be elected.
	cfg.disconnect(leader2)
	logger.Info(fmt.Sprintf("disconnect leader %d", leader2))
	cfg.disconnect((leader2 + 1) % servers)
	logger.Info(fmt.Sprintf("disconnect leader %d", (leader2+1)%servers))
	time.Sleep(2 * RaftElectionTimeout)
	...
	logger.Info(fmt.Sprintf("connect leader %d", (leader2+1)%servers))
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	logger.Info(fmt.Sprintf("connect leader %d", leader2))
	cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		i3 := rand.Int() % servers
		cfg.disconnect(i1)
		logger.Info(fmt.Sprintf("disconnect server %d", i1))
		cfg.disconnect(i2)
		logger.Info(fmt.Sprintf("disconnect server %d", i2))
		cfg.disconnect(i3)
		logger.Info(fmt.Sprintf("disconnect server %d", i3))
		...
		cfg.connect(i1)
		logger.Info(fmt.Sprintf(" connect server %d", i1))
		cfg.connect(i2)
		logger.Info(fmt.Sprintf("connect server %d", i2))
		cfg.connect(i3)
		logger.Info(fmt.Sprintf("connect server %d", i3))
	}
...
func TestFigure82C(t *testing.T) {
	...
	logger.Warn(fmt.Sprintf("agree on a cmd succeed"))
	nup := servers
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	cfg.one(rand.Int(), servers, true)
	logger.Error("aggree on one command succeed")
	leader1 := cfg.checkOneLeader()
