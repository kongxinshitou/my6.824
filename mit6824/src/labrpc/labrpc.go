...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	case e.ch <- req:
		...
		logger.Info(fmt.Sprintf("send request success"))
	case <-e.done:
...
// the coroutine to process the req
func (rn *Network) processReq(req reqMsg) {
	...
	if enabled && servername != nil && server != nil {
		...
		// if not connected sleep for some time, if rn.longDelays enabled sleep 0-7000 seconds, else sleep 0 - 100 seconds
		ms := 0
		if rn.longDelays {
